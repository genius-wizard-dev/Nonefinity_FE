...
interface KnowledgeStoreState {
  ...
  compatibleKnowledgeStores: KnowledgeStore[];
  ...
  // UI State
  loading: boolean;
  loadingCompatibleStores: boolean;
  error: string | null;
  ...
  devtools(
    subscribeWithSelector((set, get) => ({
      ...
      fetchKnowledgeStores: async (params = {}) => {
        ...
        if (currentState.loading && !params?.background) return; // Prevent concurrent calls if not background
        ...
        try {
          ...
          const now = Date.now();
          const isFresh = now - (currentState.lastFetchTime || 0) < TTL_MS;
          const usingDefaultPage =
            paginationParams.skip === currentState.pagination.skip &&
            paginationParams.limit === currentState.pagination.limit;
          ...
          if (
            ...
            if (!params?.background) set({ loading: false });
            (async () => {
              try {
      ...
      fetchKnowledgeByDimension: async (dimension: number) => {
        set({ loadingCompatibleStores: true, compatibleError: null });
        try {
          const response = await KnowledgeStoreService.getKnowledgeByDimension(
            dimension
          );
          set({
            compatibleKnowledgeStores: response,
            loadingCompatibleStores: false,
            compatibleError: null,
          });
        } catch (error) {
      ...
      fetchStoresByDimension: async (dimension: number) => {
        ...
        try {
          ...
          set({
            compatibleKnowledgeStores: response, // response is already an array
            loadingCompatibleStores: false,
            compatibleError: null,
          });
        } catch (error) {
...
export const useCompatibleKnowledgeStores = () =>
  useKnowledgeStoreStore((state) => state.compatibleKnowledgeStores);
export const useLoadingCompatibleStores = () =>
  useKnowledgeStoreStore((state) => state.loadingCompatibleStores);
...
export const useCompatibleError = () =>
  useKnowledgeStoreStore((state) => state.compatibleError);
