...
interface ModelState {
    ...
    // Caching
    lastFetchTime: number; // epoch ms for list caching
    ...
    devtools(
        (set, get) => ({
            ...
            fetchModels: async (params?: ModelListParams) => {
                ...
                try {
                    const { filters, pagination, lastFetchTime, models } =
                        get();
                    ...
                    const now = Date.now();
                    const isFresh = now - (lastFetchTime || 0) < TTL_MS;
                    const noExplicitParams =
                    ...
                    if (
                        ...
                        // Serve cached models immediately and revalidate in background
                        set({ loading: false });
                        (async () => {
                            try {
                                ...
                                console.warn(
                                    "Background revalidation (models) failed:",
                                    e
                                );
                            }
                        })();
            ...
            deleteModel: async (modelId: string) => {
                ...
                try {
                    ...
                    if (result.success) {
                        ...
                        throw new Error(
                            result.error || "Failed to delete model"
                        );
                    }
                } catch (error) {
                    // Revert on error
            ...
            fetchModelCredential: async (credentialId: string) => {
                ...
                try {
                    const response = await ModelService.getModelCredential(
                        credentialId
                    );
                    if (response) {
                        set({
                            modelCredentials: response,
                            modelCredentialLoading: false,
                        });
                    } else {
                        set({
...
export const useModelSelectors = () => {
    ...
    return {
        ...
        filteredModels: (store.models || []).filter((model) => {
            ...
            if (credential_id && model.credential_id !== credential_id)
                return false;
        ...
        chatModels: (store.models || []).filter(
