...
export function FileCard({
  ...
  useEffect(() => {
    ...
    window.addEventListener("closeAllInfoDialogs", handleCloseAllInfoDialogs);
    return () => {
  ...
  const handleRename = () => {
    if (newName.trim() && newName !== getFileNameWithoutExtension(file.name)) {
  ...
  return (
    ...
    >
      ...
      <FileIcon fileName={file.name} fileExt={file.ext} className="h-12 w-12" />
        ...
        {isRenaming ? (
          <div className="flex flex-col items-center gap-1">
            <Input
              ...
              className="h-7 w-28 text-sm text-center transition-all duration-200 shadow-none outline-none focus:outline-none focus:ring-0 bg-white text-black border border-black"
              autoFocus
          ...
          <div className="flex flex-col items-center gap-1">
            <p className="h-7 flex items-center text-sm font-medium text-black truncate max-w-24">
              {getFileNameWithoutExtension(file.name)}
            </p>
            <span className="text-xs text-muted-foreground">
              {getFileExtension(file.name)}
