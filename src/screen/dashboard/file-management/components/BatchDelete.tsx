...
export const BatchDelete: React.FC<BatchDeleteProps> = ({
  ...
  const performBatchDelete = async (ids: string[]) => {
    try {
      const response = await api.post(ENDPOINTS.FILES.BATCH_DELETE_ALT, {
        file_ids: ids,
      });
      ...
      if (
        ...
        const getResultId = (r: any) => r?.fileId ?? r?.file_id ?? r?.id;
    ...
    for (let i = 0; i < ids.length; i += batchSize) {
      ...
      for (const fileId of chunk) {
        ...
        try {
          ...
          await api.delete(ENDPOINTS.FILES.DELETE_ALT(fileId));
      ...
      <DialogContent className="sm:max-w-[600px] max-h-[80vh] overflow-y-auto">
        <DialogHeader>
          <DialogTitle className="flex items-center gap-2">
            ...
            {isDeleting ? "Deleting Files..." : "Confirm Batch Delete"}
          </DialogTitle>
          <DialogDescription>
            {isDeleting ? (
              <>Deleting {fileIds.length} files. This may take a moment...</>
            ) : (
              <>
                ...
                {fileIds.length === 1 ? "file" : "files"} from the server.
              </>
            )}
          </DialogDescription>
        </DialogHeader>
          ...
          {isDeleting && (
            <div className="space-y-2">
              ...
              <div className="flex items-center justify-between text-xs text-muted-foreground">
                <span>
                  {successCount + errorCount} of {fileIds.length} processed
                </span>
              ...
              <div className="flex items-start gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-muted-foreground">
                    Once deleted, these files cannot be recovered. Make sure you
            ...
            <div className="max-h-60 overflow-y-auto border rounded-lg">
              <div className="divide-y">
                {fileIds.map((fileId) => {
                  ...
                  const result = deleteResults.find((r) => r.fileId === fileId);

                  return (
                    <div key={fileId} className="flex items-center gap-3 p-3">
      ...
      if (fileIds.length <= batchSize) {
        ...
        for (let i = 0; i < fileIds.length; i += batchSize) {
          ...
          for (const fileId of chunk) {
            ...
            completed++;
            const progressPercent = (completed / fileIds.length) * 100;
