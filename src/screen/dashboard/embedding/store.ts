    ...
    devtools(
        subscribeWithSelector((set, get) => ({
            ...
            createFileEmbedding: async (data: CreateFileEmbeddingRequest) => {
                ...
                try {
                    const response = await EmbeddingService.createFileEmbedding(
                        data
                    );
            ...
            createTextEmbedding: async (data: CreateTextEmbeddingRequest) => {
                ...
                try {
                    const response = await EmbeddingService.createTextEmbedding(
                        data
                    );
            ...
            refreshTaskStatus: async (taskId: string) => {
                try {
                    const taskData = await EmbeddingService.getTaskStatus(
                        taskId
                    );
                ...
                try {
                    await EmbeddingService.pollTaskStatus(taskId, {
                        ...
                        onProgress: (task) => {
                            ...
                            // Update toast
                            const toastId =
                                currentState.activeToasts.get(taskId);
                            if (toastId) {
                                toast.dismiss(toastId);
                            }
                            ...
                            const newActiveToasts = new Map(
                                currentState.activeToasts
                            );
                            ...
                            if (correctedTask.ready) {
                                ...
                                if (correctedTask.successful) {
                                    ...
                                    const currentState = get();
                                    const updatedTasks = currentState.tasks.map(
                                        (t) =>
                                            t.task_id === taskId
                                                ? correctedTask
                                                : t
            ...
            fetchTasks: async (params?: TasksListRequest) => {
                const TTL_MS = 30_000; // 30s cache TTL
                set({ loading: true, error: null });
                ...
                try {
                    ...
                    const effectiveLimit =
                        params?.limit ?? currentState.pageSize;

                    const now = Date.now();
                    const isFresh =
                        now - (currentState.lastFetchTime || 0) < TTL_MS;
                    const noFilters = !params?.status && !params?.task_type;
                    const isSamePage =
                        effectiveSkip ===
                            currentState.currentPage * currentState.pageSize &&
                        effectiveLimit === currentState.pageSize;

                    // Serve cached data immediately if fresh and relevant, then revalidate in background
                    if (
                        isFresh &&
                        noFilters &&
                        isSamePage &&
                        ...
                        set({ loading: false });
                        // Background revalidation (does not flip loading)
                        (async () => {
                            try {
                                const response =
                                    await EmbeddingService.getTasks({
                                        skip: effectiveSkip,
                                        limit: effectiveLimit,
                                    });
                                const frontendTasks: EmbeddingTask[] =
                                    response.tasks.map((backendTask) => ({
                    ...
                    // Otherwise perform network fetch normally
            ...
            refreshTasks: async () => {
                ...
                const activeTasksArray = Array.from(
                    currentState.activeTasks.values()
                );
            ...
            deleteTask: async (taskId: string) => {
                ...
                try {
                    ...
                    if (result.deleted) {
                        ...
                        // Find the backend task by either Mongo _id or Celery task_id
                        const matchedBackend = currentState.backendTasks.find(
                            (t) => t._id === taskId || t.task_id === taskId
                        );

                        // Remove from backendTasks list (match both id variants to be safe)
                        const updatedBackendTasks =
                            currentState.backendTasks.filter(
                                (t) => t._id !== taskId && t.task_id !== taskId
                            );

                        // Remove from frontend tasks list using the task_id
                        const taskIdToRemove =
                            matchedBackend?.task_id || taskId;
                        const updatedFrontendTasks = currentState.tasks.filter(
                            (t) => t.task_id !== taskIdToRemove
                        );

                        set({
                            backendTasks: updatedBackendTasks,
                            tasks: updatedFrontendTasks,
                            totalTasks: Math.max(
                                0,
                                currentState.totalTasks - 1
                            ),
                            loading: false,
                            error: null,
                        });
                        ...
                        // 🔄 Optionally refresh in background to stay in sync with backend
                        get()
                            .refreshTasks()
                            .catch((e) =>
            ...
            clearTasks: async (
                ...
                try {
                    ...
                    toast.success(
                        `Successfully deleted ${result.deleted_count} task(s)`
                    );
                } catch (error) {
                    set({
                        loading: false,
